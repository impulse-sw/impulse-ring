...
static void buf_byte(buf *b, uint8_t v) { buf_push(b, &v, 1); }
...
static int64_t avro_get_long_r(ir_avro_r *r) {
    uint64_t zz = 0;
    if (avro_get_varint(r, &zz)) return 0;
...
void ir_avro_put_null(ir_avro_w *w) { (void)w; }
void ir_avro_put_boolean(ir_avro_w *w, int v) { buf_byte(&w->b, v ? 1 : 0); }
void ir_avro_put_int(ir_avro_w *w, int32_t v) { avro_put_long(&w->b, v); }
void ir_avro_put_long(ir_avro_w *w, int64_t v) { avro_put_long(&w->b, v); }
...
void ir_avro_put_string(ir_avro_w *w, const char *s) { avro_put_str(&w->b, s, strlen(s)); }
void ir_avro_put_bytes(ir_avro_w *w, const uint8_t *b, size_t n) {
    avro_put_long(&w->b, (int64_t)n);
    buf_push(&w->b, b, n);
}
void ir_avro_array_start(ir_avro_w *w, int64_t count) { avro_put_long(&w->b, count); }
void ir_avro_array_end(ir_avro_w *w) { avro_put_long(&w->b, 0); }
...
void ir_avro_r_free(ir_avro_r *r) { free(r); }
int ir_avro_get_boolean(ir_avro_r *r) { return r->pos < r->len ? r->p[r->pos++] : 0; }
int32_t ir_avro_get_int(ir_avro_r *r) { return (int32_t)avro_get_long_r(r); }
int64_t ir_avro_get_long(ir_avro_r *r) { return avro_get_long_r(r); }
...
char *ir_avro_get_string(ir_avro_r *r) {
    ...
    if (n < 0 || r->pos + (size_t)n > r->len) n = 0;
...
int64_t ir_avro_get_array_count(ir_avro_r *r) { return avro_get_long_r(r); }
...
static inline uint32_t *r_u32(ring *r, size_t off) { return (uint32_t *)(r->base + off); }
static inline uint64_t *r_u64(ring *r, size_t off) { return (uint64_t *)(r->base + off); }
static inline uint8_t *r_data(ring *r) { return r->base + RING_HEADER; }

static int ring_attach(ring *r, uint8_t *base) {
    uint32_t magic = __atomic_load_n((uint32_t *)(base + R_MAGIC), __ATOMIC_ACQUIRE);
    if (magic != RING_MAGIC) return -1;
...
static void ring_unlock(ring *r) {
    uint32_t *lock = r_u32(r, R_PRODLOCK);
    if (__atomic_exchange_n(lock, 0, __ATOMIC_RELEASE) == 2) futex_wake(lock, 1);
}
...
static int ring_push_locked(ring *r, const uint8_t *data, size_t len) {
    ...
    if ((size_t)(r->cap - (head - tail)) < need) return 1; /* full */
...
static int ring_push(ring *r, const uint8_t *data, size_t len, int timeout_ms) {
    ...
    for (;;) {
        ...
        if (timeout_ms >= 0) {
            ...
            if (elapsed >= timeout_ms) return IR_ERR_TIMEOUT;
...
static size_t ring_try_pop(ring *r, uint8_t **out) {
    ...
    size_t len = (size_t)lb[0] | ((size_t)lb[1] << 8) | ((size_t)lb[2] << 16) |
                 ((size_t)lb[3] << 24);
...
static uint8_t *frame_encode(uint64_t fp, const uint8_t *body, size_t body_len, size_t *out_len) {
    ...
    f[2] = WIRE_VERSION;
    f[3] = 0;
    for (int i = 0; i < 8; i++) f[4 + i] = (uint8_t)(fp >> (8 * i));
...
static int frame_decode(const uint8_t *f, size_t len, uint64_t *fp, const uint8_t **body,
                        size_t *body_len) {
    if (len < FRAME_HEADER) return -1;
    if ((f[0] | (f[1] << 8)) != FRAME_MAGIC) return -1;
    if (f[2] != WIRE_VERSION) return -1;
    uint64_t v = 0;
    for (int i = 0; i < 8; i++) v |= (uint64_t)f[4 + i] << (8 * i);
    uint32_t bl = (uint32_t)f[12] | ((uint32_t)f[13] << 8) | ((uint32_t)f[14] << 16) |
                  ((uint32_t)f[15] << 24);
    if (FRAME_HEADER + bl > len) return -1;
    *fp = v;
    *body = f + FRAME_HEADER;
    *body_len = bl;
    return 0;
...
const char *ir_last_error(ir_conn *c) { return c ? c->err : "no connection"; }
void ir_free(void *p) { free(p); }
...
static int64_t rand_i64(void) {
    ...
    if (fd >= 0) {
        ...
        close(fd);
        if (got == (ssize_t)sizeof v) return (int64_t)v;
    }
    /* fallback */
    struct timespec ts;
...
static int slot_wait(slot *s, int timeout_ms, uint64_t *fp, uint8_t **body, size_t *len) {
    ...
    int rc = 0;
    while (!s->done && rc == 0) rc = pthread_cond_timedwait(&s->cv, &s->m, &ts);
    int ret;
    if (s->done) {
        *fp = s->fp;
        *body = s->body;
...
static int control_call(ir_conn *c, uint64_t fp, const uint8_t *body, size_t body_len,
                        int64_t corr, int timeout_ms, uint64_t *rfp, uint8_t **rbody,
                        size_t *rlen) {
    ...
    if (rc != 0) slot_take(c, corr); /* remove if still pending */
    slot_free(s);
    return rc;
}
...
ir_conn *ir_connect(const char *app_name) {
    ...
    char reply_name[256];
    snprintf(reply_name, sizeof reply_name, "/impulse-ring.cli.%llu.v1",
             (unsigned long long)(uint64_t)nonce);
...
fail:
    ...
    c->running = 0;
    if (c->dispatcher) pthread_join(c->dispatcher, NULL);
    seg_close(&c->reply);
    seg_close(&c->ctl);
    {
...
ir_publisher *ir_publish_channel(ir_conn *c, const char *name, const char *schema_json,
                                 const char *key) {
    int64_t corr = rand_i64();
    ir_avro_w *w = ir_avro_w_new();
    ir_avro_put_long(w, corr);
...
int ir_list_channels(ir_conn *c, ir_channel_info *out, size_t max, size_t *count) {
    ...
    while ((block = ir_avro_get_array_count(r)) != 0) {
        if (block < 0) block = -block; /* negative count => block-with-size form */
...
uint64_t ir_subscriber_schema_fp(const ir_subscriber *s) { return s->schema_fp; }

/* ---- service ---- */
static ring *caller_ring(service *sv, const char *name) {
    caller_seg *cs = sv->cache;
    while (cs) {
        if (strcmp(cs->name, name) == 0) return &cs->ring;
        cs = cs->next;
    }
...
static void *service_main(void *arg) {
    ...
    while (*sv->running) {
        ...
        if (rr && frame) ring_push(rr, frame, flen, 2000);
        free(frame);
        ir_avro_w_free(w);
        free(result);
        free(reply_seg);
...
int ir_expose_function(ir_conn *c, const char *name, const char *req_schema_json,
                       const char *resp_schema_json, const char *key, ir_handler handler,
                       void *user) {
    int64_t corr = rand_i64();
    ir_avro_w *w = ir_avro_w_new();
    ir_avro_put_long(w, corr);
    ...
    if (seg_open(req_arena, &sv->arena) != 0 ||
        ring_attach(&sv->req_ring, (uint8_t *)sv->arena.addr) != 0) {
...
int ir_call(ir_conn *c, const char *fn_name, const char *key, const uint8_t *req, size_t req_len,
    ...
    free(rbody);
    if (status != ST_OK) {
        int code = (status == ST_DENIED) ? IR_ERR_DENIED
                   : (status == ST_NOT_FOUND) ? IR_ERR_NOTFOUND
                                              : IR_ERR;
