...
static int add_handler(const uint8_t *req, size_t req_len, uint8_t **resp, size_t *resp_len,
                       void *user) {
    (void)user;
...
int main(void) {
    ...
    /* expose add() */
    CHECK(ir_expose_function(svc, "c-add", ADD_REQ_SCHEMA, ADD_RESP_SCHEMA, "fn-key",
                             add_handler, NULL) == IR_OK,
          "expose function");
