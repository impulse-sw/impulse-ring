...
use impulse_core::avro::Fingerprint;
use impulse_core::proto::{self, status, ChannelInfo};
...
mod tests {
    ...
    fn key_acl() {
        ...
        assert_eq!(
            check_key(&Some(h), "wrong").unwrap_err().0,
            status::ERR_DENIED
        );
    }
    ...
    fn subscribe_resolution() {
        ...
        // wrong key denied
        assert_eq!(
            r.resolve_subscribe(cid, "bad", 0).unwrap_err().0,
            status::ERR_DENIED
        );
        // right key, wrong fp
        assert_eq!(
        ...
        // unknown channel
        assert_eq!(
            r.resolve_subscribe(999, "k", 0).unwrap_err().0,
