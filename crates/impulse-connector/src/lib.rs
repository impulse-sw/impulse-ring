...
impl Connection {
    ...
    pub fn connect(app_name: &str) -> io::Result<Connection> {
        ...
        let reply_seg = Arc::new(Segment::create(
            &reply_name,
            ring_bytes(control::REPLY_CAP),
        )?);
        ...
        let dispatcher =
            std::thread::spawn(move || client::run_dispatcher(d_inner, reply_ring, d_run));
