...
impl Ring {
  ...
  pub fn attach(seg: Arc<Segment>, base: usize) -> io::Result<Ring> {
    ...
    let capacity = unsafe { seg.atomic_u32_at(base + OFF_CAP) }.load(Ordering::Relaxed) as usize;
  ...
  fn write_wrapped(&self, pos: u64, src: &[u8]) {
    ...
    unsafe {
      ...
      if first < src.len() {
        std::ptr::copy_nonoverlapping(src.as_ptr().add(first), self.data_ptr(), src.len() - first);
      }
    }
  }
...
mod tests {
  ...
  fn temp_ring(cap: usize) -> Ring {
    ...
    let name = format!("/impulse-ring.test.{}.{}.v1", std::process::id() as u64, uniq);
